...
class MSMA:
    ...
    def calculate_silhoutte(self,location,labels):
        ...
        self.result=np.loadtxt(open("C:\personal\PhD\Dataset\\sonar-data-set\\sonar.all-data.csv", "r"), delimiter=",")
            ...
            for itr in range(100):
                ...
                for key1 in range(len(singlechild)):
                    ...
                    if count  < mutantThreshold and randomnumber%2==0:
                     mutantList.insert(key1, rand.randint(1, 2))
                     count=count+1
                    else:
    ...
    def get_dunn_score(self, population):
        dunnscore_dict = {}
        for row in range(len(population)):
            print('population[row]',population[row])
    ...
    def start_clustering(self):
        ...
        for outerstage in range(20):
         counter = 0
         parent_dict = {}
         survivor_dict = {}
         newpopluation_array_outer = util.convert_dict_to_list(sorted_dict, len(sorted_dict), self.datapoint_number)
         newpopluation_array_outer = self.delete_identical_row(newpopluation_array_outer)
         ...
         if len(offspring_dict)==0 | len(mutant_dict)==0:
            ...
            for innerstage in range(10):
                ...
                newpopluation_array_first = self.delete_identical_row(newpopluation_array_first)
...
if __name__ == '__main__':
    test_labels = np.ones(208, int)
    test_labels[0:97] = np.int(2)
   ...
   # print("kmeans without meme", confusion_matrix(test_labels, kmeans1.labels_))
    ...
    M1, C1 = kMedoids(D1, 2)
    kmediodlabels1 = convertLabelsToList(M1, C1, 208)
