...
class MSMA:
    ...
    def calculate_silhoutte(self,location,labels):
        ...
        self.result=np.loadtxt(open("C:\personal\PhD\Dataset\Anomaly\KnowledgeModel\\real_6.csv", "r"),
                         delimiter=",")
            ...
            for itr in range(100):
                ...
                for key1 in range(len(singlechild)):
                    ...
                    if count  < mutantThreshold and randomnumber%2==0:
                     mutantList.insert(key1, rand.randint(0, 3))
                     count=count+1
                    else:
...
if __name__ == '__main__':
    test_labels = np.ones(145, int)
    test_labels[0:26] = np.int(0)
    test_labels[26:65] = np.int(2)
    test_labels[65:111] = np.int(3)
    ...
    result1=np.loadtxt(open("C:\personal\PhD\Dataset\Anomaly\KnowledgeModel\\real_6.csv", "r"),
                         delimiter=",")
    kmeans1 = KMeans(n_clusters=4, random_state=0).fit(result1)
   # print("kmeans without meme", confusion_matrix(test_labels, kmeans1.labels_))
    ...
    M1, C1 = kMedoids(D1, 4)
    kmediodlabels1 = convertLabelsToList(M1, C1, 145)
