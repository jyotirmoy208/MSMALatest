...
class MSMA:
    ...
    def create_population(self):
        ...
        for row in range(self.version_number):
            #for col in range(self.datapoint_number):
                test = np.zeros(360, int)
                a=rand.randint(1,n)
                test[0:a] = np.int(1)
                b = rand.randint(1, n)
                test[a:a+b] = np.int(2)
                c = rand.randint(1, n)
                test[a+b:a + b+c] = np.int(3)
                d = rand.randint(1, n)
                test[a + b+c:a + b + c+d] = np.int(4)
                e = rand.randint(1, n)
                test[a + b + c+d:a + b + c + d+e] = np.int(5)
                f = rand.randint(1, n)
                test[a + b + c + d+e:a + b + c + d + e+f] = np.int(6)
                g = rand.randint(1, n)
                test[a + b + c + d + e+f:a + b + c + d + e + f+g] = np.int(7)
                h = rand.randint(1, n)
                test[a + b + c + d + e + f+g:a + b + c + d + e + f + g+h] = np.int(8)
                i = rand.randint(1, n)
                test[a + b + c + d + e + f + g+h:a + b + c + d + e + f + g + h+i] = np.int(9)
                j = rand.randint(1, n)
                test[a + b + c + d + e + f + g + h+i:a + b + c + d + e + f + g + h + i+j] = np.int(10)
                k = rand.randint(1, n)
                test[a + b + c + d + e + f + g + h + i+j:a + b + c + d + e + f + g + h + i + j+k] = np.int(11)
                l = rand.randint(1, n)
                test[a + b + c + d + e + f + g + h + i + j+k:a + b + c + d + e + f + g + h + i + j + k+l] = np.int(12)
                m = rand.randint(1, n)
                test[a + b + c + d + e + f + g + h + i + j + k+l:a + b + c + d + e + f + g + h + i + j + k + l+m] = np.int(13)
                o = rand.randint(1, n)
                test[a + b + c + d + e + f + g + h + i + j + k + l+m:a + b + c + d + e + f + g + h + i + j + k + l + m+o] = np.int(
            14)
                population[row]=test
    ...
    def calculate_silhoutte(self,location,labels):
        ...
        self.result=np.loadtxt(open("C:\personal\PhD\Dataset\Anomaly\Libras\\real_6.csv", "r"),
                         delimiter=",")
    ...
    def get_crossover(self,parent_dict):
        ...
        for key in parent_dict:
            if iterator == 0:
                ...
                crossoverThreshold = math.floor(len(parent_dict[key]) / 2)
            elif iterator % 2 == 0:
                father[key] = parent_dict[key]
            else:
        ...
        for key in childDict:
            ...
            for itr in range(100):
                ...
                for key1 in range(len(singlechild)):
                    randomnumber=rand.randint(0, 200)
                    if count  < mutantThreshold and randomnumber%4==0:
                     mutantList.insert(key1, rand.randint(0, 14))
                     count=count+1
                    else:
...
if __name__ == '__main__':
    test_labels = np.zeros(360, int)
    test_labels[0:24] = np.int(1)
    test_labels[24:48] = np.int(2)
    test_labels[48:72] = np.int(3)
    test_labels[72:96] = np.int(4)
    test_labels[96:120] = np.int(5)
    test_labels[120:144] = np.int(6)
    test_labels[144:168] = np.int(7)
    test_labels[168:192] = np.int(8)
    test_labels[192:216] = np.int(9)
    test_labels[216:240] = np.int(10)
    test_labels[240:264] = np.int(11)
    test_labels[264:288] = np.int(12)
    test_labels[288:312] = np.int(13)
    test_labels[312:336] = np.int(14)
    ...
    result1=np.loadtxt(open("C:\personal\PhD\Dataset\Anomaly\Libras\\real_6.csv", "r"),
                         delimiter=",")
